...
test('Docker end-to-end: encrypted egress, child TCP/DNS isolation, Unix socket denial, hidden dotenv, and workspace edits', {
  skip: process.env.HECC_TEST_DOCKER !== '1', timeout: 60_000,
}, async t => {
  ...
  await writeFile(path.join(workspace, '.env'), 'HECC_API_KEY=synthetic-detector-key', { mode: 0o600 });
  ...
  `;
  const binary = await realpath(process.env.HECC_TEST_CLAUDE ?? path.join(homedir(), '.local/bin/claude'));
  ...
  assert.equal(captured.length, 1); assert.ok(!captured[0].includes('synthetic-secret')); assert.ok(captured[0].includes('[[HECC:v1:'));
  assert.equal(await readFile(path.join(workspace, 'result.txt'), 'utf8'), 'sandbox checks passed');
  assert.equal(await readFile(path.join(workspace, '.env'), 'utf8'), 'HECC_API_KEY=synthetic-detector-key');
});

test('native Claude end-to-end through sandbox and mock upstream includes file and tool-result protection', {
  skip: process.env.HECC_TEST_DOCKER !== '1', timeout: process.env.HECC_TEST_LIVE === '1' ? 900_000 : 120_000,
}, async t => {
  const runtime = await temporary(t); const workspace = await temporary(t);
  ...
  const live = process.env.HECC_TEST_LIVE === '1' ? await providerSettings(await loadConfig(configDirectory())) : undefined;
