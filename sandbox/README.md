...
`hecc sandbox-build` builds `hecc-sandbox:0.3.0` from this directory. The build
context contains only `Dockerfile` and the compiled `scripts/relay.ts` runtime,
never workspace data or keys. Run the build through HECC to assemble that context.
...
It retains the default deny behavior and architecture-specific rules. HECC removes
...
The rest of HECC is [MIT licensed](../LICENSE).
